...
def transform_dictfields_to_list_of_tuples(record, dct, ENV=False):
    ...
    :param sleektiv.models.Model record: The model object.
...
def get_model_for_read(model, ENV=False):
    ...
    if not test_mode:
        ...
        # Contrary to ISOLATION_LEVEL_SERIALIZABLE as per Sleektiv Standard
...
def get_dict_from_record(
    ...
    :param sleektiv.models.Model record: The singleton record to load.
    ...
    for field in _spec:
        ...
        # Normal field, or unspecified relational
            ...
            if not hasattr(record, field):
                raise sleektiv.exceptions.ValidationError(
                    sleektiv._('The model "%s" has no such field: "%s".')
                    % (record._name, field)
                )
