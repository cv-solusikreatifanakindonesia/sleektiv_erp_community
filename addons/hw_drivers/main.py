...
class Manager(Thread):
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to Sleektiv database
        """
        server = helpers.get_sleektiv_server_url()
        if server:
            subject = helpers.read_file_first_line('sleektiv-subject.conf')
            if subject:
            ...
            _logger.warning('Sleektiv server not set')
