...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        The logic will only be applied if flectrabot is in a chat with a user or
        if someone pinged flectrabot.

         :param record: the mail_thread (or mail_channel) where the user
            message was posted/flectrabot will answer.
        ...
        if len(record) != 1 or values.get("author_id") == flectrabot_id:
            return
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        if self._is_bot_in_private_channel(record):
            # main flow
            if flectrabot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.flectrabot_state = "onboarding_command"
                self.env.user.flectrabot_failed = False
                return _("Great! 👍<br/>To access special commands, <b>start your sentence with</b> <span class=\"o_flectrabot_command\">/</span>. Try getting help.")
            ...
            # easter eggs
            elif flectrabot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
                ...
                if flectrabot_state == 'onboarding_emoji':
                    ...
                    return _("Not exactly. To continue the tour, send an emoji: <b>type</b> <span class=\"o_flectrabot_command\">:)</span> and press enter.")
                elif flectrabot_state == 'onboarding_attachement':
                    self.env.user.flectrabot_failed = True
                    return _("To <b>send an attachment</b>, click on the <i class=\"fa fa-paperclip\" aria-hidden=\"true\"></i> icon and select a file.")
                elif flectrabot_state == 'onboarding_command':
                    self.env.user.flectrabot_failed = True
                ...
                return random.choice([
                    ...
                    _("Sorry I'm sleepy. Or not! Maybe I'm just trying to hide my unawareness of human language...<br/>I can show you features if you write: <span class=\"o_flectrabot_command\">start the tour</span>.")
                ])
        return False
    ...
    def _is_bot_pinged(self, values):
        flectrabot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        return flectrabot_id in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        flectrabot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        if record._name == 'mail.channel' and record.channel_type == 'chat':
            return flectrabot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
    ...
    def _is_help_requested(self, body):
        ...
        """
        return any(token in body for token in ['help', _('help'), '?']) or self.env.user.flectrabot_failed
