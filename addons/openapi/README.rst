...
with whatever system you need. All can be configured in Sleektiv UI, no extra module
is needed.
...
As a workaround for multi-db Sleektiv instances, system uses `Basic Authentication <https://swagger.io/docs/specification/2-0/authentication/basic-authentication/>`__ with
...
* TODO: when user is not authenticated api returns 200 with the message below, instead of designed 401
  ...
  ::
    ...
    File "/opt/sleektiv/vendor/it-projects-llc/sync-addons/openapi/controllers/pinguin.py", line 152, in authenticate_token_for_user
...
* TODO: ``.../swagger.json`` url doesn't work in multi-db mode in sleektiv 12.0 at least: it make strange redirection to from ``/api/v1/demo/swagger.json?token=demo_token&db=source`` to ``/api/v1/demo/swagger.json?token%3Ddemo_token%26db%3Dsource``
...
* `IT-Projects LLC <https://it-projects.info>`__
      ...
      To get a guaranteed support you are kindly requested to purchase the module at `sleektiv apps store <https://store.flectrahq.com/apps/modules/13.0/openapi/>`__.
