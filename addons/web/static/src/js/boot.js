...
(function () {
    ...
    sleektiv.testing = typeof QUnit === 'object';
    sleektiv.remainingJobs = jobs;
    sleektiv.__DEBUG__ = {
    ...
    sleektiv.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__sleektiv_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: sleektiv.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = sleektiv.__DEBUG__.getDependencies(job.name);
            ...
            missing = sleektiv.__DEBUG__.getMissingJobs();
            failed = sleektiv.__DEBUG__.getFailedJobs();
            ...
            if (sleektiv.debug || failed.length || unloaded.length) {
                ...
                if (sleektiv.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        sleektiv.__DEBUG__.jsModules = {
            missing: missing,
    ...
    sleektiv.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            sleektiv.processJobs(jobs, services);
