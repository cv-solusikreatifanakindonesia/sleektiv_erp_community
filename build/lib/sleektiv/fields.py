...
from sleektiv.exceptions import CacheMiss
...
class Field(MetaField('DummyField', (object,), {})):
    ...
    :param str group_operator: aggregate function used by :meth:`~sleektiv.models.Model.read_group`
    ...
    def mapped(self, records):
        ...
        over a simple call to `~sleektiv.models.BaseModel.mapped()` on a recordset.
        """
        if self.name == 'id':
            # not stored in cache
...
class Float(Field):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~sleektiv.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~sleektiv.fields.Float.round()` to round a float with the given precision.
    :func:`~sleektiv.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~sleektiv.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field):
    ...
    :class:`res_currency<sleektiv.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <sleektiv.addons.base.models.res_currency.Currency>`
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-sleektiv: currency may not be already initialized if it is
...
class Image(Binary):
    ...
    :param bool verify_resolution: whether the image resolution should be verified
        ...
        See :class:`sleektiv.tools.image.ImageProcess` for maximum image resolution (default: ``50e6``).
...
class Many2one(_Relational):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~sleektiv.models.Model._check_company`. Add a default company
