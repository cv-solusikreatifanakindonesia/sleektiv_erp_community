...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('sleektiv.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='sleektiv')
...
DEFAULT_LOG_CONFIGURATION = [
    'sleektiv.http.rpc.request:INFO',
    'sleektiv.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['sleektiv:DEBUG', 'sleektiv.sql_db:INFO', 'sleektiv.http.rpc:DEBUG'],
    'debug_rpc': ['sleektiv:DEBUG', 'sleektiv.sql_db:INFO', 'sleektiv.http.rpc.request:DEBUG'],
    'debug': ['sleektiv:DEBUG', 'sleektiv.sql_db:INFO'],
    'debug_sql': ['sleektiv.sql_db:DEBUG'],
    'info': [],
    'runbot': ['sleektiv:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['sleektiv:WARNING', 'werkzeug:WARNING'],
    'error': ['sleektiv:ERROR', 'werkzeug:ERROR'],
    'critical': ['sleektiv:CRITICAL', 'werkzeug:CRITICAL'],
}
