...
import sleektiv.release
import sleektiv.sql_db
import sleektiv.tools
from sleektiv.sql_db import db_connect
from sleektiv.release import version_info
...
def check_super(passwd):
    if passwd and sleektiv.tools.config.verify_admin_password(passwd):
        return True
    raise sleektiv.exceptions.AccessDenied()

# This should be moved to sleektiv.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = sleektiv.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            sleektiv.modules.db.initialize(cr)
            sleektiv.tools.config['load_language'] = lang
            cr.commit()

        registry = sleektiv.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = sleektiv.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = sleektiv.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = sleektiv.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = sleektiv.tools.config['db_template']
    ...
    if sleektiv.tools.config['unaccent']:
        try:
            db = sleektiv.sql_db.db_connect(name)
            with closing(db.cursor()) as cr:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    sleektiv.sql_db.close_db(db_original_name)
    db = sleektiv.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = sleektiv.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = sleektiv.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = sleektiv.tools.config.filestore(db_original_name)
    to_fs = sleektiv.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    sleektiv.modules.registry.Registry.delete(db_name)
    sleektiv.sql_db.close_db(db_name)
    ...
    fs = sleektiv.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'sleektiv_dump': '1',
        'db_name': cr.dbname,
        'version': sleektiv.release.version,
        'version_info': sleektiv.release.version_info,
        'major_version': sleektiv.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = sleektiv.tools.config.filestore(db_name)
            ...
            sleektiv.tools.exec_pg_command(*cmd)
            if stream:
                sleektiv.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                sleektiv.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = sleektiv.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        if sleektiv.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = sleektiv.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = sleektiv.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    sleektiv.modules.registry.Registry.delete(old_name)
    sleektiv.sql_db.close_db(old_name)
    ...
    old_fs = sleektiv.tools.config.filestore(old_name)
    new_fs = sleektiv.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    sleektiv.tools.config.set_admin_password(new_password)
    sleektiv.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        sleektiv.tools.config['update']['base'] = True
        sleektiv.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@sleektiv.tools.mute_logger('sleektiv.sql_db')
...
def list_dbs(force=False):
    if not sleektiv.tools.config['list_db'] and not force:
        raise sleektiv.exceptions.AccessDenied()

    if not sleektiv.tools.config['dbfilter'] and sleektiv.tools.config['db_name']:
        ...
        res = sorted(db.strip() for db in sleektiv.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [sleektiv.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if sleektiv.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        sleektiv.sql_db.close_db(database_name)
...
def exp_list_lang():
    return sleektiv.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(sleektiv.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return sleektiv.release.version
