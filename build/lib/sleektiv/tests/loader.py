...
from .. import tools
from .common import TagsSelector, FlectraSuite
from .runner import FlectraTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules('sleektiv.addons', module)

    try:
        importlib.import_module('sleektiv.upgrade.%s' % module)
    except ImportError:
        pass
    else:
...
def _get_upgrade_test_modules(module):
    upg = importlib.import_module("sleektiv.upgrade")
    for path in map(Path, upg.__path__):
        for test in (path / module / "tests").glob("test_*.py"):
            spec = importlib.util.spec_from_file_location(f"sleektiv.upgrade.{module}.tests.{test.stem}", test)
            if not spec:
                continue
...
def make_suite(module_names, position='at_install'):
    ...
    return FlectraSuite(sorted(tests, key=lambda t: t.test_sequence))
