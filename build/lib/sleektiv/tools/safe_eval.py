...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    except sleektiv.exceptions.UserError:
        raise
    except sleektiv.exceptions.RedirectWarning:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except sleektiv.http.AuthenticationError:
        raise
...
`sleektiv.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `sleektiv.tools.safe_eval`.
""")
    return d
