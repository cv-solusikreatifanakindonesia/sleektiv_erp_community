...
class FlectraTestTimeoutError(Exception):
    pass


class FlectraTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=7073, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise FlectraTestError("Installation of package failed")
        else:
        ...
        raise FlectraTestError("Package test: FAILED. Not able to install base.")
...
class Docker():
    ...
    def test_sleektiv(self):
        ...
        if self.is_running():
            self.stop()
            raise FlectraTestTimeoutError('Flectra pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise FlectraTestError('Error while installing/starting Flectra after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
