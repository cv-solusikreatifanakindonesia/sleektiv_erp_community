...
from sleektiv.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    def button_uninstall(self):
        un_installable_modules = set(sleektiv.conf.server_wide_modules) & set(self.mapped('name'))
    ...
    def install_from_urls(self, urls):
        ...
        OPENERP = sleektiv.release.product_name.lower()
        ...
        try:
            ...
            if urls.get(OPENERP):
                ...
                # copy all modules in the SERVER/sleektiv/addons directory to the new "sleektiv" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'sleektiv' subdirectory ?
                ...
                _logger.info('Copy downloaded module `sleektiv` to `%s`', server_dir)
            ...
            if installed or to_install:
                ...
                self._cr.commit()
                sleektiv.service.server.restart()
                return {
                    'type': 'ir.actions.client',
                    'tag': 'home',
                    'params': {'wait': True},
