...
from sleektiv.tests.common import BaseCase, SavepointCase, TransactionCase
...
class EmailConfigCase(SavepointCase):
    @patch.dict("sleektiv.tools.config.options", {"email_from": "settings@example.com"})
    ...
    def test_email_from_rewrite(self):
        ...
        set_param('mail.catchall.domain', 'sleektiv.example.com')
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', 'notification@sleektiv.example.com')
        set_param('mail.catchall.domain', 'sleektiv.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(email_from, '"Admin" <notification@sleektiv.example.com>',
                         msg='Domain is not the same as the catchall domain, we should force the email FROM')
        self.assertEqual(return_path, 'notification@sleektiv.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@sleektiv.example.com>')
        self.assertEqual(email_from, '"Admin" <admin@sleektiv.example.com>',
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', '"Your notification bot" <notification@sleektiv.example.com>')
        set_param('mail.catchall.domain', 'sleektiv.example.com')
