...
from sleektiv.exceptions import AccessDenied, AccessError
from sleektiv.http import _request_stack

import sleektiv.tools
from sleektiv.tests import common
from sleektiv.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, sleektiv.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
