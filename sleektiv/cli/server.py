...
__author__ = sleektiv.release.author
__version__ = sleektiv.release.version

# Also use the `sleektiv` logger for the main script.
_logger = logging.getLogger('sleektiv')
...
def report_configuration():
    ...
    _logger.info('addons paths: %s', sleektiv.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = sleektiv.tools.config
    if not sleektiv.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = sleektiv.modules.registry.Registry.new(dbname)
        with sleektiv.api.Environment.manage():
            with registry.cursor() as cr:
                sleektiv.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with sleektiv.api.Environment.manage():
        with registry.cursor() as cr:
            sleektiv.tools.trans_load(
                cr, config["translate_in"], config["language"], overwrite=overwrite,
            )

def main(args):
    check_root_user()
    sleektiv.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                sleektiv.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except sleektiv.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        sleektiv.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = sleektiv.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the sleektiv server (default command)"""
    def run(self, args):
        main(args)
