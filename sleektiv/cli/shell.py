...
class Shell(Command):
    """Start sleektiv in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        sleektiv.cli.server.report_configuration()
        sleektiv.service.server.start(preload=[], stop=True)
        ...
        with sleektiv.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = sleektiv.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = sleektiv.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
