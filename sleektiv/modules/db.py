...
def is_initialized(cr):
    ...
    """
    return sleektiv.tools.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    f = sleektiv.modules.get_module_resource('base', 'data', 'base_data.sql')
    if not f:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with sleektiv.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in sleektiv.modules.get_modules():
        mod_path = sleektiv.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = sleektiv.modules.load_information_from_description_file(i)
