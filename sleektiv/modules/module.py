...
import sleektiv
import sleektiv.tools as tools
import sleektiv.release as release
...
# ad_paths is a deprecated alias, please use sleektiv.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"sleektiv.modules.module.ad_paths" is a deprecated proxy to '
        '"sleektiv.addons.__path__".', DeprecationWarning, stacklevel=2)
    return sleektiv.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "sleektiv.addons".',
    ...
    def load_module(self, name):
        ...
        sleektiv_name = re.sub(r'^openerp.addons.(\w+)$', r'sleektiv.addons.\g<1>', name)

        sleektiv_module = sys.modules.get(sleektiv_name)
        if not sleektiv_module:
            sleektiv_module = importlib.import_module(sleektiv_name)

        sys.modules[name] = sleektiv_module

        return sleektiv_module

class FlectraHook(object):
    """ Makes sleektiv package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to sleektiv.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'sleektiv\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `sleektiv.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^sleektiv\.addons\.base\.maintenance\.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("sleektiv.addons.base.maintenance.migrations", "sleektiv.upgrade")
...
def initialize_sys_path():
    ...
    """
    # hook sleektiv.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in sleektiv.addons.__path__:
        sleektiv.addons.__path__.append(dd)

    # hook sleektiv.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in sleektiv.addons.__path__:
            sleektiv.addons.__path__.append(ad)

    # hook sleektiv.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in sleektiv.addons.__path__ and os.path.isdir(base_path):
        sleektiv.addons.__path__.append(base_path)

    # hook sleektiv.upgrade on upgrade-path
    from sleektiv import upgrade
    ...
    # create decrecated module alias from sleektiv.addons.base.maintenance.migrations to sleektiv.upgrade
    spec = importlib.machinery.ModuleSpec("sleektiv.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["sleektiv.addons.base.maintenance"] = maintenance_pkg
    sys.modules["sleektiv.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to sleektiv and "crm"-like to sleektiv.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in sleektiv.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = sleektiv.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in sleektiv.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    try:
        __import__('sleektiv.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['sleektiv.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in sleektiv.addons.__path__:
