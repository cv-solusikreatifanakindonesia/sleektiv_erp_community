...
import sleektiv
from sleektiv.modules import get_modules
from sleektiv.modules.registry import Registry
from sleektiv.release import nt_service_name
from sleektiv.tools import config
from sleektiv.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'sleektiv.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The FLECTRA_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("FLECTRA_MAX_HTTP_THREADS")
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(sleektiv, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in sleektiv.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="sleektiv.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from sleektiv.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = sleektiv.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(sleektiv.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="sleektiv.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="sleektiv.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(sleektiv, 'phoenix', None):
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = sleektiv.tests.runner._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('sleektiv: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(sleektiv.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = sleektiv.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('sleektiv.netsvc.rpc.request')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(sleektiv.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            sleektiv.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if sleektiv.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    from sleektiv.tests.common import FlectraSuite
        ...
        try:
            ...
            if config['test_file']:
                ...
                if not os.path.isfile(test_file):
                    ...
                    with sleektiv.api.Environment.manage():
                        load_test_file_py(registry, test_file)

            # run post-install tests
            if config['test_enable']:
                t0 = time.time()
                t0_sql = sleektiv.sql_db.sql_counter
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             sleektiv.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the sleektiv http server and cron processor.
    """
    global server
    ...
    if sleektiv.evented:
        server = GeventServer(sleektiv.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(sleektiv.service.wsgi_server.application)
        ...
        server = ThreadedServer(sleektiv.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not sleektiv.evented:
        if inotify:
    ...
    if getattr(sleektiv, 'phoenix', False):
        _reexec()
