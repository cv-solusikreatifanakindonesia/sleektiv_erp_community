...
import sleektiv
from sleektiv.tools import config, topological_sort, unique
from sleektiv.netsvc import init_logger
from sleektiv.tests import standalone_tests
import sleektiv.tests.loader

_logger = logging.getLogger('sleektiv.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with sleektiv.api.Environment.manage():
        with sleektiv.registry(db_name).cursor() as cr:
            env = sleektiv.api.Environment(cr, sleektiv.SUPERUSER_ID, {})
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of Flectra modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Flectra modules")
...
def test_full(args):
    ...
    with sleektiv.api.Environment.manage():
        with sleektiv.registry(args.database).cursor() as cr:
            env = sleektiv.api.Environment(cr, sleektiv.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = sleektiv.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        sleektiv.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        sleektiv.tools.config['addons_path'] = ','.join([args.addons_path, sleektiv.tools.config['addons_path']])
        if args.data_dir:
            sleektiv.tools.config['data_dir'] = args.data_dir
        sleektiv.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('sleektiv.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('sleektiv.sql_db').setLevel(logging.CRITICAL)
