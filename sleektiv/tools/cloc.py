...
import sleektiv
from sleektiv.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in sleektiv.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [sleektiv.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(sleektiv.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be sleektiv/studio unless it comes from an imported module install
        ...
        for f in env['ir.model.fields'].browse(data.keys()):
            self.book(
                data[f.id][0] or "sleektiv/studio",
    ...
    def count_database(self, database):
        with sleektiv.api.Environment.manage():
            registry = sleektiv.registry(config['db_name'])
            with registry.cursor() as cr:
                uid = sleektiv.SUPERUSER_ID
                env = sleektiv.api.Environment(cr, uid, {})
                self.count_env(env)
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="Flectra cloc", lines="Line", other="Other", code="Code")
        s += hr
