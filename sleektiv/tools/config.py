#sleektiv.loggers.handlers. -*- coding: utf-8 -*-
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.sleektivrc (or to ~/.openerp_serverrc if it exists)")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'sleektiv_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "sleektiv.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="sleektiv.http.rpc.request:DEBUG", help='shortcut for --log-handler=sleektiv.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="sleektiv.http.rpc.response:DEBUG", help='shortcut for --log-handler=sleektiv.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="sleektiv.http:DEBUG", help='shortcut for --log-handler=sleektiv.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="sleektiv.sql_db:DEBUG", help='shortcut for --log-handler=sleektiv.sql_db:DEBUG')
    ...
    def parse_config(self, args=None):
        ...
        This method initializes sleektiv.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            sleektiv.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        sleektiv.netsvc.init_logger()
        self._warn_deprecated_options()
        sleektiv.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'sleektiv.conf')
        else:
            rcfilepath = os.path.expanduser('~/.sleektivrc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.sleektivrc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.sleektivrc'.")
