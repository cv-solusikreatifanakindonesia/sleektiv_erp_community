...
import sleektiv
import sleektiv.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from sleektiv.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if sleektiv.tools.config['db_host']:
        env['PGHOST'] = sleektiv.tools.config['db_host']
    if sleektiv.tools.config['db_port']:
        env['PGPORT'] = str(sleektiv.tools.config['db_port'])
    if sleektiv.tools.config['db_user']:
        env['PGUSER'] = sleektiv.tools.config['db_user']
    if sleektiv.tools.config['db_password']:
        env['PGPASSWORD'] = sleektiv.tools.config['db_password']
    return env
...
def _fileopen(path, mode, basedir, pathinfo, basename=None, filter_ext=None, env=None):
    ...
    paths = sleektiv.addons.__path__ + [config['root_path']]
    if env and hasattr(env.all, '__file_open_tmp_paths'):
...
def file_open_temporary_directory(env):
    ...
    Examples::
        ...
        >>> with sleektiv.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with sleektiv.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = sleektiv.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('sleektiv.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('sleektiv.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = sleektiv.fields.Datetime.from_string(value)
            value = sleektiv.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = sleektiv.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = sleektiv.fields.Datetime.from_string(value)
    else:
        timestamp = value
