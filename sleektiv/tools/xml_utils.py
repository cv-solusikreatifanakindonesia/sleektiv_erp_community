...
class sleektiv_resolver(etree.Resolver):
    """Flectra specific file resolver that can be added to the XML Parser.
...
def _check_with_xsd(tree_or_str, stream, env=None):
    ...
    :param env (sleektiv.api.Environment): If it is given, it enables resolving the
    ...
    if env:
        parser.resolvers.add(sleektiv_resolver(env))
